...
class OpenStreetMapService:
    """Сервис для работы с OpenStreetMap API"""
    ...
    def get_city_boundaries(city_name):
        ...
        try:
            ...
            print(f"Nominatim: Ошибка запроса к Nominatim API при получении границ города: {e}")
            return None
        except json.JSONDecodeError:
            print(f"Nominatim: Ошибка декодирования JSON ответа Nominatim API.")
            if 'response' in locals() and response is not None:
                print(f"Nominatim: Raw Nominatim API response (JSONDecodeError): {response.text[:500]}...")
            return None
        except Exception as e:
            print(f"Nominatim: Непредвиденная ошибка в get_city_boundaries: {e}")
            traceback.print_exc()
            return None
    ...
    def get_districts_in_city(city_name):
        """
        ИСПРАВЛЕННАЯ ВЕРСИЯ: Получить районы города через GeoJSON из Nominatim
        """
        districts_data = []
        ...
        try:
            ...
            # Сначала попробуем получить районы напрямую через Nominatim с GeoJSON
            ...
            # Если Nominatim не дал результатов, используем старый метод через Overpass
    ...
    def _get_districts_via_nominatim(city_name):
        ...
        # Известные районы Бишкека на разных языках
        district_names = [
        ...
        for district_query in district_names:
            if len(found_districts) >= 4:  # У Бишкека 4 района
                break
            ...
            try:
                ...
                time.sleep(1)  # Соблюдаем лимиты Nominatim
                ...
                for item in data:
                    if (item.get('osm_type') == 'relation' and 
                        ...
                        if district_name and district_name not in found_districts:
                            ...
                            # Получаем GeoJSON геометрию
                            geojson = item.get('geojson')
                            if geojson:
                                geometry_coords = OpenStreetMapService._convert_geojson_to_leaflet(geojson)
                                
                                if geometry_coords:
                                    center_lat, center_lng = OpenStreetMapService._calculate_polygon_center(geometry_coords)
                                    
                                    # Определяем плотность населения
    ...
    def _extract_district_name(nominatim_item):
        """Извлечь нормализованное имя района из ответа Nominatim"""
        # Пробуем разные поля с именами
        display_name = nominatim_item.get('display_name', '')
        name = nominatim_item.get('name', '')
        
        # Список возможных имен районов
        district_patterns = [
    ...
    @staticmethod
    def _convert_geojson_to_leaflet(geojson):
        """Конвертировать GeoJSON геометрию в формат для Leaflet"""
        geometry_coords = []
        ...
        try:
            ...
            if geometry_type == 'Polygon':
                # Простой полигон: [[[lng, lat], [lng, lat], ...]]
                for ring in coordinates:
                    if ring and len(ring) >= 3:
                        # Конвертируем [lng, lat] в [lat, lng] для Leaflet
                        leaflet_coords = [[point[1], point[0]] for point in ring if len(point) >= 2]
                        if len(leaflet_coords) >= 3:
                            geometry_coords.append(leaflet_coords)
                            
            elif geometry_type == 'MultiPolygon':
                # Мультиполигон: [[[[lng, lat], [lng, lat], ...]], [...]]
            ...
            print(f"Конвертировано {geometry_type} в {len(geometry_coords)} полигонов для Leaflet")
    ...
    def _get_districts_via_overpass(city_name):
        """Fallback метод через Overpass API (старая версия)"""
        ...
        # Упрощенный запрос для получения базовой геометрии
...
out geom;"""
        ...
        try:
            ...
            # Обрабатываем результат (упрощенная версия)
            districts_data = []
            temp_densities = {'default': 4000}
            ...
            for element in data.get('elements', []):
                if element['type'] == 'relation' and 'tags' in element:
                    ...
                    # Простое извлечение геометрии (может быть неполным)
                    geometry_coords = OpenStreetMapService._extract_relation_geometry(element)
                    if geometry_coords:
                        center_lat, center_lng = OpenStreetMapService._calculate_polygon_center(geometry_coords)
    ...
    @staticmethod
    def _extract_relation_geometry(relation_element):
        """Простое извлечение геометрии (оригинальный метод)"""
        geometry_coords = []
            ...
            if isinstance(geometry, list) and len(geometry) > 0:
                if isinstance(geometry[0], dict) and 'lat' in geometry[0] and 'lon' in geometry[0]:
                    ...
                    for point in geometry:
                        current_polygon.append([point['lat'], point['lon']])
                    ...
                    for contour in geometry:
                        if isinstance(contour, list) and len(contour) > 0:
                            if isinstance(contour[0], dict) and 'lat' in contour[0]:
                                current_polygon = []
    ...
    def get_schools_in_city(city_name):
        """Получает школы в указанном городе"""
        schools_data = []
